...
|세은||
|민수||||
|종욱||보드 작성 시 유저 목록에 자기 자신도 포함된다.<br> detail 글에서 내용이 textarea로 보이는데, 자바스크립트로 익숙한 에디터를 사용할 수 있다.<br> 이동 페이지가 지금은 숫자를 입력해야만 넘어가는데, 페이지네이션으로 구현하고 싶다.<br>|registboard.jsp에서 if 조건문을 추가한다.<br>Wysiwyg 웹 편집기 종류를 이용한다.
|유진||
